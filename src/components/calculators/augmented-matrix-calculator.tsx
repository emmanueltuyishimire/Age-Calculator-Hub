...
// --- RREF Algorithm ---
function augmentedRREF(matrix: number[][]): number[][] {
  ...
  for (let r = 0; r < rowCount; r++) {
    ...
    for (let j = 0; j < rowCount; j++) {
      if (j !== r) {
        let val2 = mat[j][lead];
        mat[j] = matrix[j].map((x, k) => x - val2 * mat[r][k]);
      }
    }
...
export default function AugmentedMatrixCalculator() {
    const MAX_DIM = 8;
    const MIN_DIM = 1;
    ...
    const performRref = () => {
        ...
        try {
            const res = augmentedRREF(matrix);
            const formattedResult = res.map(row => 
                row.map(cell => {
                    try {
                        const f = fraction(cell);
                        if (f.d !== 1 && Math.abs(f.n) < 1000 && f.d < 1000) {
