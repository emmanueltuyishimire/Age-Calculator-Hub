...
const formSchema = z.object({
  ...
  compounding: z.enum(['annually', 'semiannually', 'quarterly', 'monthly', 'daily']),
...
export default function CdCalculator() {
  ...
  function onSubmit(values: FormData) {
    const { initialDeposit, interestRate, compounding, depositLengthYears, depositLengthMonths = 0, taxRate = 0 } = values;
    
    const n = getCompoundingPeriods(compounding);
    const totalMonths = depositLengthYears * 12 + depositLengthMonths;
            ...
            <div className="space-y-4">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormField control={form.control} name="interestRate" render={({ field }) => (<FormItem><FormLabel>Interest Rate (APY %)</FormLabel><FormControl><Input type="number" step="0.01" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="compounding" render={({ field }) => (<FormItem><FormLabel>Compounding</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent>
                        <SelectItem value="daily">Daily</SelectItem><SelectItem value="monthly">Monthly</SelectItem><SelectItem value="quarterly">Quarterly</SelectItem><SelectItem value="semiannually">Semiannually</SelectItem><SelectItem value="annually">Annually</SelectItem>
            ...
            <div className="flex flex-col space-y-4">
                ...
                <div className="space-y-4 animate-fade-in">
                    ...
                    <div className="space-y-1 text-sm">
                        ...
                        {result.totalTax > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Total Tax:</span><span className="font-semibold text-destructive">-{currencySymbol}{result.totalTax.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>}
                    </div>
                </div>
                ) : (
                    <div className="flex items-center justify-center h-full bg-muted rounded-lg p-8">
                        <div className="text-center"><DollarSign className="mx-auto h-12 w-12 text-muted-foreground" /><p className="mt-4 text-muted-foreground">Your results will appear here.</p></div>
                    </div>
                )}
            </div>
        </div>
        {result && (
             <div className="mt-8">
                ...
                <div className="h-[250px] w-full mb-4">
                    <ResponsiveContainer><BarChart data={chartData}><XAxis dataKey="name" tick={{ fontSize: 12 }}/><YAxis tickFormatter={(val) => `${currencySymbol}${val/1000}k`} tick={{ fontSize: 12 }} /><Tooltip formatter={(value: number) => `${currencySymbol}${value.toLocaleString(undefined, {maximumFractionDigits: 0})}`} contentStyle={{ background: 'hsl(var(--background))', border: '1px solid hsl(var(--border))' }}/><Bar dataKey="Balance" fill="hsl(var(--chart-1))" name="Balance" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
                </div>
                <div className="h-[300px] overflow-y-auto border rounded-md">
                <Table><TableHeader className="sticky top-0 bg-secondary"><TableRow><TableHead>Period</TableHead><TableHead className="text-right">Interest</TableHead><TableHead className="text-right">Tax</TableHead><TableHead className="text-right">Ending Balance</TableHead></TableRow></TableHeader>
                    <TableBody>
                        {result.schedule.map(row => (
                            <TableRow key={row.period}>
                                <TableCell>{row.period}</TableCell>
                                <TableCell className="text-right">{currencySymbol}{row.interest.toFixed(2)}</TableCell>
                                <TableCell className="text-right">{currencySymbol}{row.tax.toFixed(2)}</TableCell>
