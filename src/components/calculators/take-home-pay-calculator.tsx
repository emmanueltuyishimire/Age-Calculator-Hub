...
export default function TakeHomePayCalculator() {
  ...
  function onSubmit(values: FormData) {
    ...
    const { grossSalary, payFrequency, filingStatus, preTaxDeductions = 0, stateTaxRate = 0, hasMultipleJobs, youngDependents = 0, otherDependents = 0 } = values;

    const periods = payPeriodsPerYear[payFrequency];
    const grossPayPerPeriod = grossSalary / periods;
    
    let adjustedGrossSalary = grossSalary - preTaxDeductions;
    let standardDeduction = yearData.standardDeduction[filingStatus as FilingStatus];

    if (hasMultipleJobs) {
      // Per W-4 Step 2c, only account for deductions and credits on one job
      // We will halve the standard deduction and tax brackets for a simplified estimate
      standardDeduction /= 2;
    }

    const childTaxCredits = (youngDependents * yearData.childTaxCredit.perChild) + (otherDependents * yearData.otherDependentCredit);
    
    // Per IRS Pub 15-T, add credits to deductions to estimate withholding
    const totalDeductions = standardDeduction + childTaxCredits;
    let annualTaxableIncome = Math.max(0, adjustedGrossSalary - totalDeductions);
    ...
    const taxableForState = grossPayPerPeriod - preTaxDeductionsPerPeriod;
    const stateTax = taxableForState * (stateTaxRate / 100);
    
    // Net Pay
    const totalDeductionsPerPeriod = federalTaxPerPeriod + socialSecurityPerPeriod + medicarePerPeriod + stateTax + preTaxDeductionsPerPeriod;
  ...
  const chartData = result ? [
    ...
    { name: 'State Tax', value: result.stateTax, fill: 'hsl(var(--chart-4))' },
