...
const ScientificCalculator = () => {
    ...
    const calculate = () => {
     ...
     const handleButtonClick = (value: string) => {
        ...
        if (isResult && !isOperatorOrSpecial) {
            setExpression('');
            setIsResult(false);
        } else if (value !== '±') { // Don't reset isResult for the toggle sign button
            setIsResult(false);
        }
  ...
  return (
    <div className="bg-slate-700 dark:bg-slate-800 border-4 border-slate-600 dark:border-slate-700 rounded-xl p-2 w-full mx-auto shadow-2xl max-w-sm sm:max-w-xl">
      <div className="bg-emerald-100/10 dark:bg-black/20 rounded p-2 mb-2 border-2 border-slate-800 dark:border-black shadow-inner">
       ...
       <div className="grid grid-cols-10 gap-2">
        <div className="grid grid-cols-5 gap-1.5 col-span-5">
          {scientificButtonsConfig.map(btn => renderButton(btn))}
        </div>
         <div className="grid grid-cols-5 grid-rows-4 gap-1.5 col-span-5">
