...
function MainPercentageCalculator() {
    ...
    const calculate = () => {
        ...
        if (!val1) {
            setVal1(((num3 / num2) * 100).toString());
        } else if (!val2) {
            setVal2((num3 / (num1 / 100)).toString());
        } else {
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <div className="flex items-center gap-2 flex-wrap">
                    <Input value={val1} onChange={(e) => setVal1(e.target.value)} placeholder="Percentage" className="flex-1 min-w-[100px]" />
                    <span className="font-semibold">% of</span>
                    <Input value={val2} onChange={(e) => setVal2(e.target.value)} placeholder="Value" className="flex-1 min-w-[100px]" />
                    <span className="font-semibold">=</span>
                    <Input value={result} onChange={(e) => setResult(e.target.value)} placeholder="Result" className="flex-1 min-w-[100px]" />
                </div>
                <div className="flex gap-2">
                    <Button onClick={calculate} className="w-full">Calculate</Button>
                    <Button onClick={reset} variant="outline" aria-label="Reset"><RefreshCcw /></Button>
                </div>
            </CardContent>
        </Card>
    );
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                 <div className="flex items-center gap-2 flex-wrap p-2 border rounded-md">
                    ...
                    <Input id="p1" value={p1} onChange={e => setP1(e.target.value)} placeholder="e.g., 10" className="w-20"/>
                    <Label htmlFor="p2">% of</Label>
                    <Input id="p2" value={p2} onChange={e => setP2(e.target.value)} placeholder="e.g., 50" className="w-20"/>
                    <Button onClick={() => setPResult1(String(parseFloat(p1) * parseFloat(p2) / 100))}>?</Button>
                    {pResult1 && <span className="font-bold text-primary">{pResult1}</span>}
                </div>
                <div className="flex items-center gap-2 flex-wrap p-2 border rounded-md">
                    <Input id="q1" value={q1} onChange={e => setQ1(e.target.value)} placeholder="e.g., 5" className="w-20"/>
                    <Label htmlFor="q2">is what % of</Label>
                    <Input id="q2" value={q2} onChange={e => setQ2(e.target.value)} placeholder="e.g., 50" className="w-20"/>
                    <Button onClick={() => setQResult(String(parseFloat(q1) / parseFloat(q2) * 100))}>?</Button>
                    {qResult && <span className="font-bold text-primary">{qResult}%</span>}
                </div>
                 <div className="flex items-center gap-2 flex-wrap p-2 border rounded-md">
                    <Input id="r1" value={r1} onChange={e => setR1(e.target.value)} placeholder="e.g., 5" className="w-20"/>
                    <Label htmlFor="r2">is</Label>
                    <Input id="r2" value={r2} onChange={e => setR2(e.target.value)} placeholder="e.g., 10" className="w-20"/>
                    <Label>% of what?</Label>
                    <Button onClick={() => setRResult(String(parseFloat(r1) / (parseFloat(r2)/100)))}>?</Button>
