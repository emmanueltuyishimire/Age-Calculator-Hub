...
export default function GcfCalculator() {
  ...
  const handleCalculate = useCallback(() => {
    ...
    const calculatedGcf = absNumbers.reduce((acc, val) => gcd(acc, val));
    setResult(calculatedGcf);
  }, [input]);
  ...
  return (
    <Card className="w-full max-w-lg mx-auto shadow-lg animate-fade-in">
      <CardHeader>
        <CardTitle className="text-center">GCF Calculator</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
            <Label htmlFor='gcf-input'>Numbers (comma-separated)</Label>
            <Input 
                id="gcf-input"
                value={input} 
                onChange={e => setInput(e.target.value)} 
                aria-label="Numbers for GCF calculation"
            />
        </div>
        <div className="flex flex-col md:flex-row gap-2">
            <Button onClick={handleCalculate} className="w-full">Calculate GCF</Button>
            <Button onClick={handleReset} variant="outline" className="w-full md:w-auto" aria-label="Reset">
                <RefreshCcw className="mr-2 h-4 w-4" /> Reset
            </Button>
            <ShareButton title="GCF Calculator" text="Find the Greatest Common Factor with this easy calculator!" url="/gcf-calculator" />
        </div>

        {result !== null && (
            <div className="p-6 bg-muted rounded-lg text-center space-y-2 animate-fade-in mt-4">
                <div>
                    <h3 className="text-lg font-medium text-muted-foreground">Greatest Common Factor (GCF):</h3>
