...
export function Footer() {
  ...
  const legalCategory = categories.find(cat => cat.name === 'Legal');
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
              ...
              <div key={category.name}>
                  ...
                  <ul className="space-y-2">
                      ...
                      {category.items.length > 5 && (
                         <li>
                           <Link href={category.href} className="text-sm text-primary hover:underline font-semibold">
                              View all...
                            </Link>
                         </li>
                      )}
            ...
            <div className="col-span-2 md:col-span-1">
              {companyCategory && (
               ...
               {legalCategory && (
                  <div>
                    <h3 className="font-bold mb-4">{legalCategory.name}</h3>
                    <ul className="space-y-2">
                      {legalCategory.items.map(item => (
                        <li key={item.href}>
                              <Link href={item.href} className="text-sm text-muted-foreground hover:text-primary">
                              {item.label}
                              </Link>
